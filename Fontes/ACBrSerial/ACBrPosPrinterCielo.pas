...
const
  ...
  CnStyles = 'styles';
  CnValue = 'value';
  CmodPrintText = 'PRINT_TEXT';
  CmodPrintImage = 'PRINT_IMAGE';
  CmodPrintMultiColumnText = 'PRINT_MULTI_COLUMN_TEXT';

  CColunasImpressoraSmartPosCielo = 32;
...
type
  ...
  EACBrPosPrinterCielo = class(Exception);

  {$IfDef ANDROID}
   TACBrPosPrinterCieloErroMsg = procedure(const MsgErro: string) of object;
  {$EndIf}
  ...
  TCieloJson = class
  private
    FStyles: TACBrInformacoes;
    FValue: String;
    FOperation: String;
  public
    constructor Create(NomeOperation: String);
    ...
    property Styles: TACBrInformacoes read FStyles;
    property Value:  String read FValue write FValue;
  end;

  { TCieloJsonImp }

  TCieloJsonImp = class(TObjectList{$IfDef HAS_SYSTEM_GENERICS}<TCieloJson>{$EndIf})
  private
    fIndex: Integer;
    function GetItem(Index: Integer): TCieloJson;
    procedure SetItem(Index: Integer; const Value: TCieloJson);
    function GetNext: Boolean;
  public
    function Add (Obj: TCieloJson): Integer;
    procedure Insert (Index: Integer; Obj: TCieloJson);
    function CheckNew(align: Integer; textsize: Integer; typeface: Integer): Boolean;
    function New(NomeOperation: String): TCieloJson;

    property Items[Index: Integer]: TCieloJson read GetItem write SetItem; default;
    property &Index: Integer read fIndex write fIndex;
    property Next: Boolean read GetNext;
  end;

  TCieloPrinters = (prnSmartPOS);

  { TACBrPosPrinterCielo }

  TACBrPosPrinterCielo = class(TACBrPosPrinterClass)
  private
    fCieloJsonImp: TCieloJsonImp;
    fModelo: TCieloPrinters;
    fPastaEntradaCielo: String;
    fPastaSaidaCielo: String;
    fSeqArqCielo: Integer;
    fSeqArqCieloQr: Integer;
    fSeqArqCieloCb: Integer;
    fIPePortaCielo: String;
    fREspostaCielo: String;
    fPrinting: Boolean;
    ...
    {$IfDef ANDROID}
     ...
     fOnErroImpressao: TACBrPosPrinterCieloErroMsg;
    {$EndIf}
  protected
    procedure Imprimir(const LinhasImpressao: String; var Tratado: Boolean);

    {$IfDef ANDROID}
     procedure EnviarPorIntent(aItem: Integer);
  ...
  public
    ...
    property CieloJsonImp: TCieloJsonImp read fCieloJsonImp;
    property Modelo: TCieloPrinters read fModelo write fModelo;
    property PastaEntradaCielo: String read fPastaEntradaCielo write fPastaEntradaCielo;
    property PastaSaidaCielo: String read fPastaSaidaCielo write fPastaSaidaCielo;
    property IPePortaCielo: String read fIPePortaCielo write fIPePortaCielo;
    property REspostaCielo: string read fREspostaCielo;
    {$IfDef ANDROID}
     property OnErroImpressao: TACBrPosPrinterCieloErroMsg read fOnErroImpressao write fOnErroImpressao;
    {$EndIf}
  end;
...
constructor TCieloJson.Create(NomeOperation: String);
begin
  inherited Create;
  FOperation := NomeOperation;
  FStyles := TACBrInformacoes.Create;
  FValue := '';
end;

destructor TCieloJson.Destroy;
begin
  FStyles.Free;
...
procedure TCieloJson.Clear;
begin
  FStyles.Clear;
  FValue := '';
end;

function TCieloJson.GetJSON: String;
var
  i, j: Integer;
  {$IfDef USE_JSONDATAOBJECTS_UNIT}
   AJSon: TJsonObject;
   JSStyles: TJsonObject;
   JSValue: TJsonArray;
  {$Else}
   AJSon: TJson;
...
begin
  {$IfDef USE_JSONDATAOBJECTS_UNIT}
   ...
   try
     AJSon[CnOperation] := FOperation;

     JSStyles:= AJSon.A[CnStyles].AddObject;

     for j := 0 to FStyles.Count-1 do
     begin
       case FStyles.Items[j].Tipo of
         tiBoolean:
           JSStyles.B[ FStyles.Items[j].Nome ] := FStyles.Items[j].AsBoolean;
         tiFloat:
           JSStyles.F[ FStyles.Items[j].Nome ] := FStyles.Items[j].AsFloat;
         tiInteger, tiInt64:
           JSStyles.I[ FStyles.Items[j].Nome ] := FStyles.Items[j].AsInteger;
       else
         JSStyles.S[ FStyles.Items[j].Nome ] := FStyles.Items[j].AsString;
       end;
     end;

     AJSon.A[CnValue].Add(FValue);
...
constructor TACBrPosPrinterCielo.Create(AOwner: TACBrPosPrinter; aScheme: String);
begin
  inherited Create(AOwner);

  fCieloJsonImp := TCieloJsonImp.Create;
  fpModeloStr := 'PosPrinterCielo';
  fModelo := prnSmartPOS;
  fPastaEntradaCielo := '';
  fPastaSaidaCielo := '';
  fIPePortaCielo := '';
...
destructor TACBrPosPrinterCielo.Destroy;
begin
  fCieloJsonImp.Free;
...
procedure TACBrPosPrinterCielo.DestroyAfterPrint;
begin
...
procedure TACBrPosPrinterCielo.Configurar;
begin
...
procedure TACBrPosPrinterCielo.AntesDecodificar(var ABinaryString: AnsiString);
begin
...
procedure TACBrPosPrinterCielo.AdicionarBlocoResposta(
  const ConteudoBloco: AnsiString);
begin
  AddCmdImprimirTexto(ConteudoBloco);
end;

procedure TACBrPosPrinterCielo.DepoisDecodificar(var ABinaryString: AnsiString);
begin
  AddCmdFechaConexaoImpressora;
  ABinaryString := fCieloJsonImp.Items[0].GetJSON;
end;

function TACBrPosPrinterCielo.TraduzirTag(const ATag: AnsiString;
...
function TACBrPosPrinterCielo.TraduzirTagBloco(const ATag,
...
function TACBrPosPrinterCielo.ComandoFonte(TipoFonte: TACBrPosTipoFonte;
  Ligar: Boolean): AnsiString;
begin
  Result := '';
end;

function TACBrPosPrinterCielo.ComandoConfiguraModoPagina: AnsiString;
begin
  Result := '';
end;

function TACBrPosPrinterCielo.ComandoPosicionaModoPagina(
  APoint: TPoint): AnsiString;
begin
  Result := '';
end;

procedure TACBrPosPrinterCielo.LerStatus(var AStatus: TACBrPosPrinterStatus);
begin
  inherited;
end;

function TACBrPosPrinterCielo.LerInfo: String;
begin
  Result := inherited LerInfo;
end;

procedure TACBrPosPrinterCielo.AddCmdAbreConexaoImpressora;
begin
  // Limpa o objeto com o CieloJson
  fCieloJsonImp.Index:= 0;
  fCieloJsonImp.Clear;
end;

procedure TACBrPosPrinterCielo.AddCmdFechaConexaoImpressora;
begin
  //
end;

procedure TACBrPosPrinterCielo.AddCmdImprimirTexto(
  const ConteudoBloco: String);
var
  align, tamanho, stilo: Integer;
begin
  if (ConteudoBloco <> '') then
  begin
    align:= IfThen(fpPosPrinter.Alinhamento = alDireita, 2,
            IfThen(fpPosPrinter.Alinhamento = alCentro, 0, 1));
    ...
    if fCieloJsonImp.CheckNew(align, tamanho, stilo) then
    begin
      with fCieloJsonImp.New(CmodPrintText) do
      begin
        if Styles.Count = 0 then
        begin
          Styles.AddField('key_attributes_align').AsInteger:=    align;
          Styles.AddField('key_attributes_textsize').AsInteger:= tamanho;
          Styles.AddField('key_attributes_typeface').AsInteger:= stilo;
        end;

        Value:= Value + ConteudoBloco;
      end;
    end else
      fCieloJsonImp.Last.Value:= fCieloJsonImp.Last.Value + ConteudoBloco;
  end;
end;

procedure TACBrPosPrinterCielo.AddCmdImprimirQrCode(const ConteudoBloco: String;
...
{$IFDEF MSWINDOWS}
  sCaminho:= TPath.Combine(TPath.GetLibraryPath, 'qrcode' + fSeqArqCieloQr.ToString + '.jpg');
{$ELSE}
  sCaminho:= TPath.Combine(TPath.GetSharedDocumentsPath, 'qrcode' + fSeqArqCieloQr.ToString + '.jpg');
{$ENDIF}
...
{$ENDIF}
    ...
    BarCodeImage.SaveToFile(sCaminho);
  finally
  ...
  with fCieloJsonImp.New(CmodPrintImage) do
  begin
    if Styles.Count = 0 then
    begin
      Styles.AddField('form_feed').AsInteger:= 1;
    end;

    Value:= Value + sCaminho;
  end;
end;

procedure TACBrPosPrinterCielo.AddCmdImprimirImagem(
...
procedure TACBrPosPrinterCielo.AddCmdImprimirBarras(const ConteudoBloco: String;
...
{$IFDEF MSWINDOWS}
  sCaminho:= TPath.Combine(TPath.GetLibraryPath, 'codbarras' + fSeqArqCieloCb.ToString + '.jpg');
{$ELSE}
  sCaminho:= TPath.Combine(TPath.GetSharedDocumentsPath, 'codbarras' + fSeqArqCieloCb.ToString + '.jpg');
{$ENDIF}
...
procedure TACBrPosPrinterCielo.AddCmdPuloDeLinhas(const Linhas: Integer);
var
  I: Integer;
  tamanho: Integer;
begin
  if (fCieloJsonImp.Count > 0) then
    if fCieloJsonImp.Last.Operation = CmodPrintText then
    begin
      for I:= 1 to Linhas do
        fCieloJsonImp.Last.Value:= fCieloJsonImp.Last.Value + #10;
    end else
    if fCieloJsonImp.Last.Operation = CmodPrintImage then
    begin
      tamanho:= 32; //ftNormal
...
procedure TACBrPosPrinterCielo.Imprimir(const LinhasImpressao: String;
  var Tratado: Boolean);
var
  I: Integer;
begin
  ...
  {$IfNDef ANDROID}
  for I:= 0 to fCieloJsonImp.Count-1 do
  begin
    if (fIPePortaCielo <> '') then
      EnviarPorTCP(fCieloJsonImp.Items[I].GetJSON)
    else
      EnviarPorTXT(fCieloJsonImp.Items[I].GetJSON);
  end;
...
procedure TACBrPosPrinterCielo.EnviarPorIntent(aItem: Integer);
var
  fUriDados:        String;
  sLinhasImpressao: String;
  intentPrint:      JIntent;
  fUriEnvio:        TACBrURI;
begin
  if aItem > (fCieloJsonImp.Count - 1) then
    Exit;

  sLinhasImpressao:= fCieloJsonImp.Items[aItem].GetJSON;

  fUriEnvio:= TACBrURI.Create(URI_SchemePrint, URI_AuthorityPrint, '');

  fUriEnvio.Params.AddField('request').AsString:=     StringToBase64(sLinhasImpressao);
  fUriEnvio.Params.AddField('urlCallback').AsString:= Self.fScheme + '://' + URI_HostPrint;
...
procedure TACBrPosPrinterCielo.HandleActivityMessage(const Sender: TObject; const M: TMessage);
var
  jsRetorno: JString;
begin
  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
          if fCieloJsonImp.Next then
          begin
            fCieloJsonImp.Index:= CieloJsonImp.Index + 1;
            EnviarPorIntent(fCieloJsonImp.Index)
          end else
          begin
            fPrinting:= False;
...
{$Else}
procedure TACBrPosPrinterCielo.EnviarPorTCP(const LinhasImpressao: String);
 var
 ...
 begin
   ...
   try
     ...
     begin
       fREspostaCielo := StringReplace(Resp, LF+'}'+LF+'{', ',', [rfReplaceAll]);
       //DEBUG
...
procedure TACBrPosPrinterCielo.EnviarPorTXT(const LinhasImpressao: String);
 var
   ArqIN, ArqOUT: String;
 ...
 begin
   inc(fSeqArqCielo);
   if fSeqArqCielo > 999 then
     fSeqArqCielo := 1;

   if (fPastaEntradaCielo = '') then
      ArqIN := ApplicationPath + 'LIO'+PathDelim+'pathIN'
    else
      ArqIN := fPastaEntradaCielo;
   ArqIN := PathWithDelim(ArqIN) + 'Comando' + IntToStrZero(fSeqArqCielo, 3) + '.txt';

   if (fPastaSaidaCielo = '') then
      ArqOUT := ApplicationPath + 'LIO'+PathDelim+'pathOUT'
    else
      ArqOUT := fPastaSaidaCielo;
   ArqOUT := PathWithDelim(ArqOUT) + 'Comando' + IntToStrZero(fSeqArqCielo, 3) + '*.txt';
...
//   if (not Ok) then
//     raise EACBrPosPrinterCielo.Create('TimeOut');
end;
{$EndIf}

{ TCieloJsonImp }

function TCieloJsonImp.CheckNew(align, textsize, typeface: Integer): Boolean;
begin
  Result:= True;

  if Count > 0 then
  begin
    if Last.Operation <> CmodPrintText then
      Exit;

    if not (Last.Styles.FieldExists('key_attributes_align')
    and Last.Styles.FieldExists('key_attributes_textsize')
    and Last.Styles.FieldExists('key_attributes_typeface')) then
      Exit;

    if (Last.Styles.Fields['key_attributes_align'].AsInteger = align)
    and (Last.Styles.Fields['key_attributes_textsize'].AsInteger = textsize)
    and (Last.Styles.Fields['key_attributes_typeface'].AsInteger = typeface) then
      Result:= False;
  end;
end;

function TCieloJsonImp.GetItem(Index: Integer): TCieloJson;
begin
  Result := TCieloJson(inherited Items[Index]);
end;

function TCieloJsonImp.GetNext: Boolean;
begin
  Result:= fIndex < (Count - 1);
end;

procedure TCieloJsonImp.SetItem(Index: Integer; const Value: TCieloJson);
begin
  inherited Items[Index] := Value;
end;

function TCieloJsonImp.Add(Obj: TCieloJson): Integer;
begin
  Result := inherited Add(Obj);
end;

procedure TCieloJsonImp.Insert(Index: Integer; Obj: TCieloJson);
begin
  inherited Insert(Index, Obj);
end;

function TCieloJsonImp.New(NomeOperation: String): TCieloJson;
begin
  Result := TCieloJson.Create(NomeOperation);
