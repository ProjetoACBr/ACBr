...
{
    $retorno = $ffi->ExtratoAPI_Inicializar(FFI::addr($handle), $iniPath, "");
...
{
    $retorno = $ffi->ExtratoAPI_Finalizar($handle->cdata);
...
{
    $retorno = $ffi->ExtratoAPI_ConfigLer($handle->cdata, $eArqConfig);
...
function ConfigLerValor($handle, $ffi, $eSessao, $eChave, &$sValor)
{
    ...
    $retorno = $ffi->ExtratoAPI_ConfigLerValor($handle->cdata, $eSessao, $eChave, $sResposta, FFI::addr($esTamanho));
...
{
    $retorno = $ffi->ExtratoAPI_ConfigGravarValor($handle->cdata, $eSessao, $eChave, $value);
    $sMensagem = FFI::new("char[535]");
...
{
    $retorno = $ffi->ExtratoAPI_ConfigGravar($handle->cdata, $eArqConfig);
    $sMensagem = FFI::new("char[535]");
...
function UltimoRetorno($handle, $ffi, $retornolib, &$sMensagem, $msgErro, $retMensagem = 0)
{
    if (($retornolib !== 0) || ($retMensagem == 1)) {
        $esTamanho = FFI::new("long");
        $esTamanho->cdata = 9048;
        $resposta = $ffi->ExtratoAPI_UltimoRetorno($handle->cdata, $sMensagem, FFI::addr($esTamanho));
...
function ExtratoAPINome($handle, $ffi, &$retornoGeral)
{
    $esTamanho = FFI::new("long");
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[535]");
    $retorno = $ffi->ExtratoAPI_Nome($handle->cdata, $sMensagem, FFI::addr($esTamanho));
...
function ExtratoAPIVersao($handle, $ffi, &$retornoGeral)
{
    $esTamanho = FFI::new("long");
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[535]");
    $retorno = $ffi->ExtratoAPI_Versao($handle->cdata, $sMensagem, FFI::addr($esTamanho));
...
function OpenSSLInfo($handle, $ffi, &$retornoGeral)
{
    ...
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[535]");
    $retorno = $ffi->ExtratoAPI_OpenSSLInfo($handle->cdata, $sMensagem, FFI::addr($esTamanho));
...
function ConsultarExtrato($handle, $ffi, $AAgencia, $AConta, $ADataInicio, $ADataFim, $APagina, $ARegistrosPorPag, &$retornoGeral)
{
    $esTamanho = FFI::new("long");
    $esTamanho->cdata = 10144768;
    $sMensagem = FFI::new("char[10144768]");

    $retorno = $ffi->ExtratoAPI_ConsultarExtrato($handle->cdata, $AAgencia, $AConta, $ADataInicio, $ADataFim, $APagina, $ARegistrosPorPag, $sMensagem, FFI::addr($esTamanho));

    if ($retorno !== 0) {
        if (UltimoRetorno($handle, $ffi, $retorno, $sMensagem, "Erro ao consultar extrato", 1) != 0)
            return -10;
    }
