...
{
    $retorno = $ffi->ExtratoAPI_Inicializar($iniPath, "");
...
{
    $retorno = $ffi->ExtratoAPI_Finalizar();
...
{
    $retorno = $ffi->ExtratoAPI_ConfigLer($eArqConfig);
...
function ConfigLerValor($ffi, $eSessao, $eChave, &$sValor)
{
    ...
    $retorno = $ffi->ExtratoAPI_ConfigLerValor($eSessao, $eChave, $sResposta, FFI::addr($esTamanho));
...
{
    $retorno = $ffi->ExtratoAPI_ConfigGravarValor($eSessao, $eChave, $value);
    $sMensagem = FFI::new("char[535]");
...
{
    $retorno = $ffi->ExtratoAPI_ConfigGravar($eArqConfig);
    $sMensagem = FFI::new("char[535]");
...
function UltimoRetorno($ffi, $retornolib, &$sMensagem, $msgErro, $retMensagem = 0)
{
    if (($retornolib !== 0) || ($retMensagem == 1)) {
        $esTamanho = FFI::new("long");
        $esTamanho->cdata = 9048;
        $resposta = $ffi->ExtratoAPI_UltimoRetorno($sMensagem, FFI::addr($esTamanho));
...
function ExtratoAPINome($ffi, &$retornoGeral)
{
    $esTamanho = FFI::new("long");
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[535]");
    $retorno = $ffi->ExtratoAPI_Nome($sMensagem, FFI::addr($esTamanho));
...
function ExtratoAPIVersao($ffi, &$retornoGeral)
{
    $esTamanho = FFI::new("long");
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[535]");
    $retorno = $ffi->ExtratoAPI_Versao($sMensagem, FFI::addr($esTamanho));
...
function OpenSSLInfo($ffi, &$retornoGeral)
{
    ...
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[535]");
    $retorno = $ffi->ExtratoAPI_OpenSSLInfo($sMensagem, FFI::addr($esTamanho));
...
function ConsultarExtrato($ffi, $AAgencia, $AConta, $ADataInicio, $ADataFim, $APagina, $ARegistrosPorPag, &$retornoGeral)
{
    $esTamanho = FFI::new("long");
    $esTamanho->cdata = 10144768;
    $sMensagem = FFI::new("char[10144768]");

    $retorno = $ffi->ExtratoAPI_ConsultarExtrato($AAgencia, $AConta, $ADataInicio, $ADataFim, $APagina, $ARegistrosPorPag, $sMensagem, FFI::addr($esTamanho));

    if ($retorno !== 0) {
        if (UltimoRetorno($ffi, $retorno, $sMensagem, "Erro ao consultar extrato", 1) != 0)
            return -10;
    }
