...
type
  ...
  TfrPrincipal = class(TForm)
    ...
    tsFluxo: TTabSheet;
    procedure ACBrNFe1Transmitted(const XML: AnsiString; HTTPResultCode: Integer);
...
begin
  ...
  begin
    wCV := CupomSempreVerde or (MessageDlg('Deseja Cupom Verde?', mtConfirmation, mbYesNo, 0) = mrYes);
    if (not wCV) then
      ImprimirNFCeAtual
    else
...
procedure TfrPrincipal.InicializarActivePages;
begin
